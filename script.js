...
const ZODIAC_EXPLANATIONS = {
    ...
    "Gemini": "Sociability, cheerfulness and quick-wittedness are on the increase, and there is an interest in all kinds of information. The Moon in Gemini favors study, intellectual pursuits, acquaintances, conferences and gatherings. However, heightened changeability and scattering your interests can prevent you from getting definite, tangible results.",
...
function updateDailyView() {
    ...
    if (signTransitions.length > 0) {
        const startStr = formatTime(signTransitions[0].startTime);
        const endStr = formatTime(signTransitions[0].endTime);
        document.getElementById('sign-timing').textContent = `${startStr} - ${endStr}`;
        
        allSignsContainer.innerHTML = '<div class="all-items-title">All Moon Signs Today:</div>';
    ...
    // Show all mansions for the day
    const allMansionsContainer = document.getElementById('all-mansions-container');
    if (mansionTransitions.length > 1) {
        allMansionsContainer.innerHTML = '<div class="all-items-title">All Moon Mansions Today:</div>';
    ...
    if (vocPeriods.length === 0) {
        vocDisplay.innerHTML = `
            ...
            <span class="voc-info-link" onclick="showVOCInfoDialog()">What is Void of Course?</span>
        `;
    } else {
        let periodsHtml = '<div class="voc-status active"><span class="voc-icon">⚠</span><span>VOC Today</span></div>';
        periodsHtml += '<div class="voc-periods">';
        vocPeriods.forEach(p => {
            periodsHtml += `<div class="voc-period">${formatTime(p.start)} - ${formatTime(p.end)} (after ${p.lastAspectPlanet})</div>`;
        });
        periodsHtml += '</div>';
        periodsHtml += '<span class="voc-info-link" onclick="showVOCInfoDialog()">What is Void of Course?</span>';
        vocDisplay.innerHTML = periodsHtml;
    }
    ...
    if (retroPlanets.length === 0) {
        retroDisplay.innerHTML = '<div class="no-retro">No planets in retrograde</div>';
    } else {
        retroDisplay.innerHTML = retroPlanets.map(p => `
            <div class="retro-item" onclick="showRetroDialog('${p.planet}')">
                <span class="retro-emoji">${p.emoji}</span>
                <div class="retro-info">
                    <div class="retro-name">${p.planet}</div>
                    <div class="retro-dates">${formatShortDate(new Date(p.retroStart))} - ${formatShortDate(new Date(p.retroEnd))}</div>
                </div>
            </div>
        `).join('');
    }
    ...
    if (isMercuryRetrograde(date)) {
        mercuryStatus.textContent = 'Mercury is Retrograde ⚠';
        mercuryStatus.classList.add('retrograde');
    } else {
        mercuryStatus.textContent = 'Mercury is Direct ✓';
        mercuryStatus.classList.remove('retrograde');
    }
}
...
function updateMonthlyCalendar() {
    ...
    // Update month title
    document.getElementById('month-title').textContent = currentMonth.toLocaleDateString('en-US', { 
        month: 'long', 
        year: 'numeric',
        timeZone: locationTimezone
    });
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        dayCell.innerHTML = `
            ...
            <div class="day-mansions">${mansionTransitions.map(t => t.mansion.number).join('-')}</div>
            <div class="day-signs">${zodiacInfo.sign.emoji}</div>
        `;
...
function showMansionDialog(mansion) {
    const dialog = document.getElementById('mansion-dialog');
    document.getElementById('mansion-dialog-title').textContent = `${mansion.number}. ${mansion.name}`;
    document.getElementById('mansion-dialog-essence').textContent = mansion.essence;
    document.getElementById('mansion-dialog-good').textContent = mansion.goodFor;
    document.getElementById('mansion-dialog-bad').textContent = mansion.badFor;
    dialog.style.display = 'flex';
}

function showZodiacDialog(signName) {
    const dialog = document.getElementById('zodiac-dialog');
    const sign = ZODIAC_SIGNS.find(s => s.name === signName);
    document.getElementById('zodiac-dialog-title').textContent = `${sign.emoji} Moon in ${signName}`;
    document.getElementById('zodiac-dialog-content').innerHTML = `<p>${ZODIAC_EXPLANATIONS[signName] || 'Information not available.'}</p>`;
    dialog.style.display = 'flex';
}
...
function showPhaseDialog(phaseName) {
    ...
    document.getElementById('phase-dialog-content').innerHTML = `<p>${PHASE_EXPLANATIONS[phaseName] || 'Information not available.'}</p>`;
    dialog.style.display = 'flex';
}

function showVOCInfoDialog() {
    const dialog = document.getElementById('voc-info-dialog');
    dialog.style.display = 'flex';
}

function showRetroDialog(planetName) {
    const dialog = document.getElementById('retro-info-dialog');
    document.getElementById('retro-dialog-title').textContent = `${planetName} Retrograde`;
    
    const retroInfo = {
        "Mercury": "Mercury retrograde affects communication, technology, travel, and contracts. It's a time to review, revise, and reflect rather than start new ventures.",
        "Venus": "Venus retrograde affects relationships, finances, and values. It's a time to reassess what you value and how you relate to others.",
        "Mars": "Mars retrograde affects energy, motivation, and action. It's a time to review your goals and how you pursue them.",
        "Jupiter": "Jupiter retrograde is a time for inner growth and philosophical reflection rather than external expansion.",
        "Saturn": "Saturn retrograde is a time to review responsibilities, structures, and long-term goals.",
        "Uranus": "Uranus retrograde brings internal changes and the need to review your relationship with freedom and change.",
        "Neptune": "Neptune retrograde intensifies intuition and is a time for spiritual reflection and releasing illusions.",
        "Pluto": "Pluto retrograde is a time for deep inner transformation and examining hidden aspects of yourself."
    };
    
    document.getElementById('retro-dialog-content').innerHTML = `<p>${retroInfo[planetName] || 'Information not available.'}</p>`;
    dialog.style.display = 'flex';
}
...
function showDayDialog(date) {
    ...
    let content = '';
    content += `<div class="day-dialog-info"><span>${phaseInfo.emoji}</span><span>${phaseInfo.name} - ${getIllumination(date)}% illuminated</span></div>`;
    content += `<div class="day-dialog-info"><span>${zodiacInfo.sign.emoji}</span><span>Moon in ${zodiacInfo.sign.name} at ${zodiacInfo.degree}°</span></div>`;
    
    mansionTransitions.forEach(trans => {
        content += `<div class="day-dialog-info"><span>🌙</span><span>Mansion ${trans.mansion.number}: ${trans.mansion.name} (${formatTime(trans.startTime)} - ${formatTime(trans.endTime)})</span></div>`;
    });
    
    if (vocPeriods.length > 0) {
        vocPeriods.forEach(p => {
            content += `<div class="day-dialog-info" style="color: hsl(var(--void-red));"><span>⚠</span><span>VOC: ${formatTime(p.start)} - ${formatTime(p.end)}</span></div>`;
        });
    }
    
    document.getElementById('day-dialog-content').innerHTML = content;
    
    document.getElementById('expand-daily-btn').onclick = () => {
        selectedDate = date;
        updateDailyView();
...
// ============================================
// SWIPE HANDLING - SMOOTH AND FIXED
// ============================================
function initSwipeHandlers() {
    ...
    swipeableCards.forEach(card => {
        ...
        const handleMove = (e) => {
            ...
            const limitedDelta = deltaX * resistance;
            const maxDelta = 80;
            const clampedDelta = Math.max(-maxDelta, Math.min(maxDelta, limitedDelta));
...
function initEventListeners() {
    ...
    ['phase', 'mansion', 'sign', 'voc'].forEach(card => {
    ...
    document.querySelectorAll('.dialog-close').forEach(btn => {
        btn.addEventListener('click', () => {
            btn.closest('.dialog-overlay').style.display = 'none';
        });
    });
    ...
    document.getElementById('sign-name').addEventListener('click', () => {
        const moonLong = getMoonLongitude(selectedDate);
        const zodiacInfo = getZodiacSign(moonLong);
        showZodiacDialog(zodiacInfo.sign.name);
    });
...
document.body.addEventListener('touchmove', function(e) {
    if (e.target.closest('.swipeable-card')) {
        // Allow horizontal swipes on cards
    }
}, { passive: true });
