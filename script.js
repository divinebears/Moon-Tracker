...
// ============================================
// RETROGRADE PLANETS DATA (Updated with correct dates)
// ============================================
const RETROGRADE_PLANETS = [
    ...
    { planet: "Mercury", emoji: "☿", retroStart: new Date(2025, 9, 9), retroEnd: new Date(2025, 9, 29) },
    ...
    { planet: "Uranus", emoji: "⛢", retroStart: new Date(2024, 8, 1), retroEnd: new Date(2025, 1, 30) },
...
function getPlanetLongitudes(date) {
    // Simplified planetary positions - in a real app, you'd use ephemeris data
    const jd = toJulianDay(date);
    const T = (jd - 2451545.0) / 36525;
    ...
    // Approximate other planets (simplified)
    const mercuryLong = (sunLong + 25 * Math.sin(T * 4.15)) % 360;
    const venusLong = (sunLong + 48 * Math.sin(T * 1.62)) % 360;
    const marsLong = (68.95 + 19.14 * T * 360) % 360;
    const jupiterLong = (34.4 + 30.35 * T) % 360;
    const saturnLong = (49.94 + 12.22 * T) % 360;
    
    return {
        sun: ((sunLong % 360) + 360) % 360,
        mercury: ((mercuryLong % 360) + 360) % 360,
        venus: ((venusLong % 360) + 360) % 360,
        mars: ((marsLong % 360) + 360) % 360,
        jupiter: ((jupiterLong % 360) + 360) % 360,
        saturn: ((saturnLong % 360) + 360) % 360
    };
}

function isAspecting(moonLong, planetLong, orb = ASPECT_ORB) {
    for (const aspect of MAJOR_ASPECTS) {
        let diff = Math.abs(moonLong - planetLong - aspect);
        if (diff > 180) diff = 360 - diff;
        if (diff <= orb) return true;
        
        diff = Math.abs(moonLong - planetLong + aspect);
        if (diff > 180) diff = 360 - diff;
        if (diff <= orb) return true;
    }
    return false;
...
function findLastAspect(date) {
    ...
    for (let i = 0; i < 72 * 4; i++) { // Every 15 minutes for 3 days
        searchDate = new Date(date.getTime() - i * 15 * 60 * 1000);
        const moonLong = getMoonLongitude(searchDate);
        
        for (const [name, long] of Object.entries(planets)) {
            if (isAspecting(moonLong, long)) {
...
function findNextSignChange(date) {
    ...
    for (let i = 0; i < 72 * 4; i++) {
        ...
        if (signIndex !== currentSignIndex) {
            ...
            let low = searchDate.getTime() - 15 * 60 * 1000;
...
function getVoidOfCourseForDay(date) {
    ...
    // Check every 30 minutes for VOC periods
    while (currentTime < endOfDay) {
        ...
        for (const [name, long] of Object.entries(planets)) {
            if (isAspecting(moonLong, long, 2)) {
                isAspectingAny = true;
                break;
            }
        }
        ...
        if (!isAspectingAny) {
            ...
            if (lastAspect && nextSignChange) {
                ...
                if (vocStart < endOfDay && vocEnd > startOfDay) {
                    ...
                    const exists = vocPeriods.some(p => 
                        Math.abs(p.start.getTime() - displayStart.getTime()) < 60000
                    );
            ...
            if (nextSignChange) {
                currentTime = new Date(nextSignChange.getTime() + 30 * 60 * 1000);
            } else {
                break;
            }
        } else {
...
function isMercuryRetrograde(date) {
    return RETROGRADE_PLANETS.filter(p => p.planet === "Mercury" && date >= p.retroStart && date <= p.retroEnd).length > 0;
}
...
function updateDailyView() {
    ...
    if (vocPeriods.length > 0) {
        ...
        if (currentVOC && isToday) {
            ...
            document.getElementById('voc-times').textContent = `Until ${formatTime(currentVOC.end)}`;
        } else {
            ...
            vocText.textContent = vocPeriods.length > 1 ? 
                `${vocPeriods.length} VOC periods today` : 
                'VOC period scheduled';
            vocTiming.style.display = 'block';
            
            const vocTimesText = vocPeriods.map(p => 
                `${formatTime(p.start)} - ${formatTime(p.end)}`
            ).join(' | ');
            document.getElementById('voc-times').textContent = vocTimesText;
        }
    } else {
        vocCard.classList.remove('voc-active');
        vocStatus.textContent = '✓ Clear';
    ...
    if (retroPlanets.length === 0) {
        ...
        // Add click handlers for Mercury retrograde explanation
    ...
    summaryGrid.innerHTML = `
        ...
        <div class="summary-item">
            ...
            <span class="summary-label">${vocPeriods.length > 0 ? 'VOC Today' : 'Moon Active'}</span>
        </div>
    `;
}
...
function updateMonthlyCalendar() {
    ...
    for (let day = 1; day <= lastDay.getDate(); day++) {
        ...
        // Get mansions for the day
        const mansionTransitions = findMansionTransitions(date);
        const mansionNames = mansionTransitions.map(t => t.mansion.name).join(', ');
        
        // Get signs for the day
        const signTransitions = findSignTransitions(date);
        const signEmojis = [...new Set(signTransitions.map(t => t.sign.emoji))].join('');
...
function showAspectDialog(aspectType) {
    currentAspectType = aspectType;
    aspectViewDate = new Date(selectedDate);
    updateAspectDialogContent();
    document.getElementById('aspect-dialog').style.display = 'flex';
}

function updateAspectDialogContent() {
    document.getElementById('aspect-dialog-title').textContent = `${currentAspectType} Details`;
    document.getElementById('aspect-current-date').textContent = formatDate(aspectViewDate);
    
    const content = document.getElementById('aspect-content');
    // Populate with aspect data for the current date
    const phaseInfo = getPhaseInfo(aspectViewDate);
    const vocPeriods = getVoidOfCourseForDay(aspectViewDate);
    
    content.innerHTML = `
        <div class="dialog-section">
            <h4>Moon Phase</h4>
            <p>${phaseInfo.emoji} ${phaseInfo.name}</p>
        </div>
        <div class="dialog-section">
            <h4>Void of Course</h4>
            <p>${vocPeriods.length > 0 ? 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Aspect dialog navigation
    document.getElementById('close-aspect-dialog').addEventListener('click', () => {
        document.getElementById('aspect-dialog').style.display = 'none';
    });
    
    document.getElementById('aspect-dialog').addEventListener('click', (e) => {
        if (e.target.id === 'aspect-dialog') {
            document.getElementById('aspect-dialog').style.display = 'none';
        }
    });
    
    document.getElementById('aspect-prev-day').addEventListener('click', () => {
        aspectViewDate = new Date(aspectViewDate.getTime() - 24 * 60 * 60 * 1000);
        updateAspectDialogContent();
    });
    
    document.getElementById('aspect-next-day').addEventListener('click', () => {
        aspectViewDate = new Date(aspectViewDate.getTime() + 24 * 60 * 60 * 1000);
        updateAspectDialogContent();
    });
});
