...
const RETROGRADE_PLANETS = [
    ...
    { planet: "Mercury", emoji: "☿", retroStart: new Date(2024, 3, 1), retroEnd: new Date(2024, 3, 25) }, // Apr 1 - Apr 25, 2024
    { planet: "Mercury", emoji: "☿", retroStart: new Date(2024, 7, 5), retroEnd: new Date(2024, 7, 28) }, // Aug 5 - Aug 28, 2024
    { planet: "Mercury", emoji: "☿", retroStart: new Date(2024, 10, 25), retroEnd: new Date(2024, 11, 15) }, // Nov 25 - Dec 15, 2024
    // Mercury retrogrades 2025
    { planet: "Mercury", emoji: "☿", retroStart: new Date(2025, 2, 14), retroEnd: new Date(2025, 3, 7) }, // Mar 14 - Apr 7, 2025
    { planet: "Mercury", emoji: "☿", retroStart: new Date(2025, 6, 18), retroEnd: new Date(2025, 7, 11) }, // Jul 18 - Aug 11, 2025
    { planet: "Mercury", emoji: "☿", retroStart: new Date(2025, 10, 9), retroEnd: new Date(2025, 10, 29) }, // Nov 9 - Nov 29, 2025
    
    // Venus retrograde 2025
    { planet: "Venus", emoji: "♀", retroStart: new Date(2025, 2, 1), retroEnd: new Date(2025, 3, 12) }, // Mar 1 - Apr 12, 2025
    ...
    // Jupiter retrograde 2024-2025
    { planet: "Jupiter", emoji: "♃", retroStart: new Date(2024, 9, 9), retroEnd: new Date(2025, 1, 4) }, // Oct 9, 2024 - Feb 4, 2025
    { planet: "Jupiter", emoji: "♃", retroStart: new Date(2025, 10, 11), retroEnd: new Date(2026, 2, 10) }, // Nov 11, 2025 - Mar 10, 2026
    
    // Saturn retrograde 2024
    { planet: "Saturn", emoji: "♄", retroStart: new Date(2024, 5, 29), retroEnd: new Date(2024, 10, 15) }, // Jun 29 - Nov 15, 2024
    // Saturn retrograde 2025
    { planet: "Saturn", emoji: "♄", retroStart: new Date(2025, 6, 13), retroEnd: new Date(2025, 10, 27) }, // Jul 13 - Nov 27, 2025
    
    // Uranus retrograde 2024-2025
    { planet: "Uranus", emoji: "⛢", retroStart: new Date(2024, 8, 1), retroEnd: new Date(2025, 0, 30) }, // Sep 1, 2024 - Jan 30, 2025
    { planet: "Uranus", emoji: "⛢", retroStart: new Date(2025, 8, 6), retroEnd: new Date(2026, 1, 4) }, // Sep 6, 2025 - Feb 4, 2026
    
    // Neptune retrograde 2024
    { planet: "Neptune", emoji: "♆", retroStart: new Date(2024, 6, 2), retroEnd: new Date(2024, 11, 7) }, // Jul 2 - Dec 7, 2024
    // Neptune retrograde 2025
    { planet: "Neptune", emoji: "♆", retroStart: new Date(2025, 6, 4), retroEnd: new Date(2025, 11, 10) }, // Jul 4 - Dec 10, 2025
    
    // Pluto retrograde 2024
    { planet: "Pluto", emoji: "♇", retroStart: new Date(2024, 4, 2), retroEnd: new Date(2024, 9, 11) }, // May 2 - Oct 11, 2024
    // Pluto retrograde 2025
    { planet: "Pluto", emoji: "♇", retroStart: new Date(2025, 4, 4), retroEnd: new Date(2025, 9, 13) } // May 4 - Oct 13, 2025
];
...
function getPlanetLongitudes(date) {
    ...
    // More accurate planetary positions using mean elements
    // Mercury - fast moving, ~4° per day
    const mercuryL = 252.2509 + 149474.0722 * T;
    const mercuryA = 0.387098;
    const mercuryE = 0.205635;
...
function findLastAspect(date) {
    ...
    // Search backward up to 3 days in 2-minute increments for precision
...
function findNextSignChange(date) {
    ...
    let searchDate = new Date(date);
    // Search forward up to 3 days in 2-minute increments
    for (let i = 0; i < 72 * 30; i++) {
        searchDate = new Date(date.getTime() + i * 2 * 60 * 1000);
...
function getVoidOfCourseForDay(date) {
    ...
    // Check every 5 minutes for more precise VOC detection
    while (currentTime < endOfDay) {
        const moonLong = getMoonLongitude(currentTime);
        ...
        if (!isAspectingAny) {
            ...
            if (lastAspect && nextSignChange) {
                ...
                if (vocStart < endOfDay && vocEnd > startOfDay) {
                    ...
                    // Check if we already have this period (within 15 min)
...
function getRetrogradePlanets(date) {
    // Normalize the date to noon for comparison
...
// ============================================
// SWIPE NAVIGATION FOR ASPECT CARDS WITH ANIMATION
// ============================================
function initSwipeNavigation() {
    document.querySelectorAll('.swipeable-card').forEach(card => {
        ...
        function animateSwipe(direction) {
            ...
            card.classList.add(direction === 'prev' ? 'swipe-right' : 'swipe-left');
        ...
        // Touch swipe support with animation
        let touchStartX = 0;
        let touchEndX = 0;
